...
import { SUCCESS_CREATE_MESSAGE, SUCCESS_UPDATE_MESSAGE } from '../..//utils/constant';
...
export const UpdateRole = (props: any) => {
  const clsRef: any = useRef();
  const [state, setState] = useState({ date: new Date(), password: '' });
  const { updateSuccess } = useUser();
  ...
  const handleDropdownChange = (event: any, newValue: any) => {
    setRoles(newValue);
  };
